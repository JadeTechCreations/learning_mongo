const MongoClient = require('mongodb').MongoClient,
    assert = require('assert'),
    Hapi = require('hapi'),
    MongoService = require('./common/mongo-service');

var url = 'mongodb://localhost:27017/learning_mongo'

var server = new Hapi.Server({
    port:8080
});

server.route( [
    // Get tour list
    {
        method: 'GET',
        path: '/api/stocks',
        config: {json: {space: 2}},
        handler: async (request, reply) => {
          var findObject = {};
          for (var key in request.query) {
            if(isNaN(request.query[key])){
              findObject[key] = request.query[key];
            } else {
              findObject[key] = parseFloat(request.query[key]);
            }
          }

          let val = await collection.find(findObject).toArray();

          return reply.response(val);
        }
    },
    // Add new tour
    {
        method: 'POST',
        path: '/api/stocks',
        handler: function(request, reply) {
            collection.insertOne(request.payload, function(error, result) {
                assert.equal(null,error);
                reply(request.payload);
            })
        }
    },
    // Get a single tour
    {
        method: 'GET',
        path: '/api/stocks/{name}',
        config: {json: {space: 2}},
        handler: function(request, reply) {
            collection.findOne({"tourName":request.params.name}, function(error, tour) {
               assert.equal(null,error);
               reply(tour);
            })
        }
    },
    // Update a single tour
    {
        method: 'PUT',
        path: '/api/stocks/{name}',
        handler: function(request, reply) {
            if (request.query.replace == "true") {
                request.payload.tourName = request.params.name;
                console.log(request.payload);
                collection.replaceOne({"tourName": request.params.name},
                                      request.payload,
                   function(error, results) {
                        collection.findOne({"tourName":request.params.name},
                            function(error, results) {
                                reply(results);
                    })
            })
            } else {
                    collection.updateOne({tourName:request.params.name},
                                    {$set: request.payload},
                                    function(error, results) {
                    collection.findOne({"tourName":request.params.name}, function(error, results) {
                        reply(results);
                })
              })
            }
        }
    },
    // Delete a single tour
    {
        method: 'DELETE',
        path: '/api/stocks/{name}',
        handler: function(request, reply) {
            collection.deleteOne({tourName:request.params.name},
                function(error, results) {
                    reply ().code(204);
            })
        }
    },
    // Home page
    {
        method: 'GET',
        path: '/',
        handler: function(request, reply) {
            const response = reply.response( "Hello world from Hapi/Mongo example.");
            response.type('text/plain');
            return response;
        }
    }
])

MongoClient.connect(url, { useNewUrlParser: true }, function(err, client) {
    assert.equal(null,err);
    console.log("connected correctly to server");
    collection = client.db('learning_mongo').collection('stocks');
    server.start(function(err) {
        console.log('Hapi is listening to http://localhost:8080')
    })
})
